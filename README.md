...
Each bar is really two: a pale bar showing how far through the window you are, with the solid usage bar inset inside it. Compare the two right-hand edges — usage short of elapsed means you're spending quota slower than the clock is running it down; usage past elapsed means you'll hit the cap before the reset. It answers “am I above 50% usage with 50% of the window left?” at a glance. The marker needs both a reset time and a window length, so it's hidden where either is missing: Antigravity publishes a reset time per model but never a window length, and its monthly credit pool has neither. It's also hidden once a tier's reset time has passed, since a snapshot that outlived its own window can't be placed on one.
